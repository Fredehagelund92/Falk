...
def compute_shares(
    ...
    return [
        {**row, "share_pct": round(_safe_float(row.get(metric)) / total * 100, 1)} for row in data
    ]
...
def calculate_variance_explained(
    ...
    if abs(total_delta) < 0.01:  # Near-zero change
        return [
            {**d, "variance_explained": 0.0, "variance_pct": 0.0, "impact_rank": 0}
...
def rank_dimensions_by_impact(
    ...
    if abs(total_delta) < 0.01:
        return [
            {"dimension": d, "variance_explained_pct": 0.0, "impact_score": 0.0} for d in dimensions
        ]
